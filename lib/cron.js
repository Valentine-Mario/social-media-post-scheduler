    ...
    cron.schedule("0 * * * *", () => {
      ...
      fbPostSchema.find({ posted: false }, async (err, fb_post) => {
        for (let fb in fb_post) {
          ...
          if (myTime.hours >= scheduledTime.hours) {
            if (fb.image == undefined) {
              ...
              let encoding = await baseEncoding.convert_url_to_base64(fb.image);
        ...
        for (let tw in tw_post) {
          ...
          if (
            ...
            if (tw.image == undefined) {
              ...
              twPostSchema.findByIdAndUpdate(
                tw._id,
                { posted: true },
                (err) => {}
              );
      ...
      IgPostModel.find({ posted: false }, async (err, ig_post) => {
        for (let ig in ig_post) {
          ...
          if (
            ...
            postToIg.PostToIg(tw.text, tw.image);
            IgPostModel.findByIdAndUpdate(
              ig._id,
              { posted: true },
              (err) => {}
            );
